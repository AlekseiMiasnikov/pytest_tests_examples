...
from data import API_USERS_SUCCESS, API_GET_METHOD_NOT_ALLOWED, API_AUTH_TOKEN_EMPTY, AUTH_DATA_LENGTH_TOKEN, \
    ...
    API_BAD_REQUEST_BAD_OFFSET, API_BAD_REQUEST_OBJECTS_ARCHIVE, AUTH_DATA_FAIL_BAD_TOKEN, API_AUTH_NOT_FOUND_TOKEN, \
    AUTH_DATA_FAIL_NOT_FOUND_ID, API_BAD_RIGHTS


@suite('Контроллер: Objects. Метод: get-all')
@parent_suite('[PYTHON][API]')
class TestApiObjectsGetAll:
    @title('get-all')
    @description('Проверка корректной работы get-all c acrhive = 0')
    def test_get_all(self, create_admin, objects):
        asserts(
            assert_data=objects.get_all({
    ...
    def test_get_all_acrhive_ok(self, create_admin, objects):
        asserts(
            assert_data=objects.get_all({
                'token': create_admin['token'],
                'limit': 1,
    ...
    def test_get_all_method_not_allowed(self, objects):
        asserts(
            assert_data=objects.get_all_method_not_allowed(),
    ...
    def test_get_all_bad_request_empty_token(self, objects):
        asserts(
            assert_data=objects.get_all({
                'token': '',
                'limit': 1,
    ...
    def test_get_all_bad_request_length_token(self, objects):
        asserts(
            assert_data=objects.get_all({
    ...
    def test_get_all_bad_request_id_minus(self, objects):
        asserts(
            assert_data=objects.get_all({
                'token': 1,
                'limit': 1,
    ...
    @title('get-all-fail-bad-request-user-id-varchar')
    @description('Проверка ошибки Bad Request bad id для get-all с символами в идентификаторе пользователя')
    def test_get_all_bad_request_user_id_varchar(self, objects):
        asserts(
            assert_data=objects.get_all({
                'token': 1,
                'limit': 1,
    ...
    def test_get_all_bad_request_bad_limit(self, objects):
        asserts(
            assert_data=objects.get_all({
                'token': 1,
    ...
    def test_get_all_bad_request_bad_offset(self, objects):
        asserts(
            assert_data=objects.get_all({
                'token': 1,
                'limit': 1,
    ...
    def test_get_all_bad_request_bad_archive(self, objects):
        asserts(
            assert_data=objects.get_all({
                'token': 1,
                'limit': 1,
    ...
    def test_get_all_bad_request_bad_rights(self, create_login_delete_user, objects):
        asserts(
            assert_data=objects.get_all({
