...
from data import API_USERS_SUCCESS, API_GET_METHOD_NOT_ALLOWED, API_AUTH_TOKEN_EMPTY, AUTH_DATA_LENGTH_TOKEN, \
    ...
    API_BAD_REQUEST_BAD_OFFSET, API_BAD_REQUEST_VENDORS_ARCHIVE, AUTH_DATA_FAIL_BAD_TOKEN, \
    API_AUTH_NOT_FOUND_TOKEN, AUTH_DATA_FAIL_NOT_FOUND_ID, API_BAD_RIGHTS


@suite('Контроллер: Vendors. Метод: get-all')
@parent_suite('[PYTHON][API]')
class TestApiVendorsGetAll:
    @title('get-all')
    @description('Проверка корректной работы get-all c acrhive = 0')
    def test_get_all(self, create_admin, vendors):
        asserts(
            assert_data=vendors.get_all({
    ...
    def test_get_all_acrhive_ok(self, create_admin, vendors):
        asserts(
            assert_data=vendors.get_all({
                'token': create_admin['token'],
                'limit': 1,
    ...
    def test_get_all_method_not_allowed(self, vendors):
        asserts(
            assert_data=vendors.get_all_method_not_allowed(),
    ...
    def test_get_all_bad_request_empty_token(self, vendors):
        asserts(
            assert_data=vendors.get_all({
                'token': '',
                'limit': 1,
    ...
    def test_get_all_bad_request_length_token(self, vendors):
        asserts(
            assert_data=vendors.get_all({
    ...
    def test_get_all_bad_request_bad_id_minus(self, vendors):
        asserts(
            assert_data=vendors.get_all({
                'token': 1,
                'limit': 1,
    ...
    def test_get_all_bad_request_bad_id_varchar(self, vendors):
        asserts(
            assert_data=vendors.get_all({
                'token': 1,
                'limit': 1,
    ...
    def test_get_all_bad_request_bad_limit(self, vendors):
        asserts(
            assert_data=vendors.get_all({
                'token': 1,
    ...
    def test_get_all_bad_request_bad_offset(self, vendors):
        asserts(
            assert_data=vendors.get_all({
                'token': 1,
                'limit': 1,
    ...
    def test_get_all_bad_request_bad_archive(self, vendors):
        asserts(
            assert_data=vendors.get_all({
                'token': 1,
                'limit': 1,
    ...
    def test_get_all_bad_request_bad_rights(self, create_login_delete_user, vendors):
        asserts(
            assert_data=vendors.get_all({
